...
export interface IDatabaseTransport {
    ...
    store<T>(name: string): IDatabaseStore<T> | null;
}

export interface IDatabaseStore<T> {
    get(id: number): Promise<T>;
    getAll(): Promise<T[]>;
    getIndex<K extends keyof T>(key: K, value: number): Promise<T[]>;
    ...
    init(schema: DatabaseSchema): void {
        this.db = new Promise((resolve, reject) => {
            ...
            req.onupgradeneeded = (event: IDBVersionChangeEvent) => {
                ...
                for (const [name, { options, indexes }] of Object.entries(schema.stores)) {
...
export class IndexedDBStore<T> implements IDatabaseStore<T> {
    ...
    async getIndex<K extends keyof T>(key: K, value: number): Promise<T[]> {
        const db = await this.db;
        const tx = db.transaction(this.name, "readonly");
        const store = tx.objectStore(this.name);
