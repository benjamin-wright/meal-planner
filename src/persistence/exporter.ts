...
import { sanitize as sanitizeItem } from "../models/items";
...
export async function importData(db: DB, data: string): Promise<void> {
  ...
  await loadDataFile(parsed, db.units(), db.categories(), db.items(), db.recipies(), db.meals(), db.extra());
}

export async function initData(db: DB): Promise<void> {
  const data = defaultData as ExportedData;
  await loadDataFile(data, db.units(), db.categories(), db.items(), db.recipies(), db.meals(), db.extra());
}
...
async function loadDataFile(
  ...
  if (data.items) {
    for (let i = 0; i < data.items.length; i++) {
      await items.put(sanitizeItem(data.items[i]));
    }
  }
